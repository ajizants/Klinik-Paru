...
class SuratController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $tanggal             = $tgl; // Pastikan $tgl sudah di-set sebelumnya
            $no                  = $this->listSM($tanggal);
            $jumlahSuratTahunIni = $no['jumlahSuratTahunIni'] ?? 0; // Gunakan null coalescing operator untuk antisipasi jika key tidak ada

            $year        = date('Y', strtotime($tanggal));
            $month       = date('m', strtotime($tanggal));
            $monthRomawi = Roman::month[$month] ?? 'I'; // Gunakan default 'I' jika tidak ditemukan

            $nomorSurat = "440.6/{$jumlahSuratTahunIni}/{$monthRomawi}/{$year}";
            // dd($nomorSurat);

            // Mengatur nilai-nilai kolom dari data validasi
            $noSurat->tanggal = $validatedData['tglTrans'];
            $noSurat->noSurat = $nomorSurat;
            ...
            return response()->json([
                'message'    => 'Data berhasil disimpan, Nomor Surat : ' . $nomorSurat,
                'lists'      => $data['lists'],
